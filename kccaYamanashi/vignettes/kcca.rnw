...
<<kcca-scores>>=
@
\caption{These are the scores from KCCA. Notice that the points
  are sorted by the $\theta_{i}$ (this is not particularly surprising,
  see the PCA comparison later).}
